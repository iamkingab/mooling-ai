...
const exportAllData = () => {
  ...
  a.href = url
  a.download = `墨灵-完整备份-${new Date().toISOString().split('T')[0]}.json`
  a.click()
...
const exportNovels = () => {
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `墨灵-小说数据-${new Date().toISOString().split('T')[0]}.json`
  a.click()
  URL.revokeObjectURL(url)
...
const exportPrompts = () => {
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `墨灵-提示词库-${new Date().toISOString().split('T')[0]}.json`
  a.click()
  URL.revokeObjectURL(url)
...
const exportGenres = () => {
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `墨灵-小说类型-${new Date().toISOString().split('T')[0]}.json`
  a.click()
  URL.revokeObjectURL(url)
...
const exportSettings = () => {
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `墨灵-系统设置-${new Date().toISOString().split('T')[0]}.json`
  a.click()
  URL.revokeObjectURL(url)
